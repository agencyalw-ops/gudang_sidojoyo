...
<div style="margin-bottom: 2rem;">
    ...
    <div class="card" style="margin-bottom: 1.5rem;">
        ...
        <form method="GET" action="/reports/kasir-performance"
            ...
            <div class="form-group" style="margin-bottom: 0;">
                ...
                <select name="month" class="form-control">
                    ...
                    @for($m = 1; $m <= 12; $m++)
                        <option value="{{ $m }}" @if((int)$month === $m) selected @endif>
                            {{ $months[$m] }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="form-group" style="margin-bottom: 0;">
                ...
                <select name="year" class="form-control">
                    @for($y = now()->year; $y >= now()->year - 5; $y--)
                        <option value="{{ $y }}" @if((int)$year === $y) selected @endif>
                            {{ $y }}
                        </option>
                    @endfor
                </select>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
        ...
        <div class="card">
            ...
            <div style="font-size: 2rem; font-weight: bold; color: var(--success);">
                Rp {{ number_format((int)$totalSales) }}
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div style="font-size: 2rem; font-weight: bold;">
                Rp {{ number_format((int)$averageSales) }}
            </div>
        </div>
            ...
            @forelse($cashierPerformance as $rank => $performance)
                ...
                <tr>
                    ...
                    <td style="font-weight: 600;">
                        {{ $performance['kasir_name'] }}
                    </td>

                    <td style="text-align: center;">
                        {{ (int)$performance['total_transactions'] }}
                    </td>

                    <td style="font-weight: bold; color: var(--success);">
                        Rp {{ number_format((int)$performance['total_sales']) }}
                    </td>

                    <td>
                        Rp {{ number_format((int)$performance['average_transaction']) }}
                    </td>

                    <td style="text-align: center;">
                        {{ (int)$performance['total_items'] }} pcs
                    </td>

                    <td>
                        <a href="/reports/kasir-detail/{{ urlencode($performance['kasir_name']) }}?month={{ $month }}&year={{ $year }}"
            ...
            @empty
                <tr>
                    <td colspan="7" style="text-align: center; color: var(--text-muted);">
                        Belum ada data kasir untuk bulan ini
                    </td>
                </tr>
            @endforelse
            </tbody>
